...
```{r}
tdata_demogr <- read_delim("Exp1_subj_explanations.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)


mean(tdata_demogr$age)
sd(tdata_demogr$age)
min(tdata_demogr$age)
max(tdata_demogr$age)
```
...
```{r}
# make machine_type a factor

tdata$machine_type <- factor(tdata$machine_type, 
                             levels = c("functional", "dysfunctional", "separate_parts"))
```
...
g1 <- ggplot(tdata, aes(x = machine_type, y = num_act, fill = machine_type)) +
  ...
  scale_x_discrete(labels = c("func.", "dysf.", "scr.")) +
...
title_text <- "Mean number of listed configuration terms"
subtitle_text <- "identified by different LLMs per machine type condition"
caption_text <- "Note. Bars show means, error bars show 95% CIs."
...
ggsave("Exp1_Results.svg",width=12,height=5)
ggsave("Exp1_Results.pdf",width=12,height=5)
